<x-layouts.dashboard>
    <div class="col-span-4" x-data="{togglePositionForm: false}">
        ...
        <div class="col-span-4 p-6 bg-white drop-shadow-sm rounded-lg mt-8">
            <form action="{{route('dashboard.users.update',$user->id)}}" method="POST" class="gap-4 grid grid-cols-2" autocomplete="off" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="space-y-2">
                    ...
                    <select name="unit_id" id="" class="focus:outline-none px-4 py-4 text-sm w-full border border-neutral-100 rounded-lg">
                        ...
                        <option @if($user->unit_id == $unit->id) selected @endif value="{{$unit->id}}">{{$unit->name}}</option>                    
                        @endforeach
                    </select>
                </div>
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{$user->name}}" placeholder="Nama Karyawan" class="focus:outline-none px-4 py-4 text-sm w-full border border-neutral-100 rounded-lg">
                </div>
                <div class="space-y-2">
                    <label class="text-sm uppercase font-medium tracking-[1px] text-neutral-600"
                    for="username">Username <span class="text-red-600">*</span></label>
                    <input autocomplete="false" type="text" name="username" id="username" value="{{$user->username}}" placeholder="Username Karyawan" class="focus:outline-none px-4 py-4 text-sm w-full border border-neutral-100 rounded-lg">
                </div>
                <div class="space-y-2">
                    <label class="text-sm uppercase font-medium tracking-[1px] text-neutral-600"
                ...
                <div class="space-y-2">
                    ...
                    <input autocomplete="false" type="date" name="join_date" id="join_date" value="{{$user->join_date}}" class="focus:outline-none px-4 py-4 text-sm w-full border border-neutral-100 rounded-lg">
                </div>
                <div class="space-y-2">
                    ...
                    <livewire:multi-select selectedPositions="{{$user->positions->pluck('id')}}"/>
                </div>
