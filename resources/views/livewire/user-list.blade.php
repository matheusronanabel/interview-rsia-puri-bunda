        ...
        <tbody>
            @foreach ($users as $user)
                <tr class="border-b border-b-neutral-100">
                    ...
                    <td class="px-6 py-3 gap-4 flex flex-wrap">
                        @foreach ($user->positions as $position)
                            <span class="px-2 py-1.5 w-fit text-xs bg-blue-100 text-blue-800 font-medium rounded-full">
                                {{$position->name}}
                            </span>
                        @endforeach
                    </td>
                    <td class="px-6 py-3">
                        @php
                            $latestLogin = \App\Models\LoginLog::where('user_id',$user->id)->latest()->first();
                        @endphp
                        {{$latestLogin->login_time ?? ''}}
                    </td>
                    <td class="px-6 py-3">
                        {{$user->created_at}}
                    </td>
                    <td class="px-6 py-3 flex gap-4">
                        <a href="{{route('dashboard.users.show',$user->id)}}" class="flex items-center space-x-2 text-neutral-400">
                            <div>
                        ...
                        <a href="{{route('dashboard.users.edit',$user->id)}}" class="flex items-center space-x-2 text-yellow-400">
                            <div>
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-3">
